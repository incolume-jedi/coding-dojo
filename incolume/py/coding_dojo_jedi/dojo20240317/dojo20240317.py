...
class Orcamento:
    ...
    def calc_vol_simples(self, capacidade: float) -> float:
...
def qlatas(
    area: float,
    recipiente: list[RecipienteTinta] | None = None,
) -> str:
    ...
    orc = Orcamento(area)
    v1 = orc.calc_vol_simples(recipiente[0].volume) * recipiente[0].valor
    v2 = orc.calc_vol_simples(recipiente[1].volume) * recipiente[1].valor
    return (
        f'Area: {area} m2;'
